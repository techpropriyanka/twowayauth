...
class Facebook
{
     ...
     */
    const APP_ID_ENV_NAME = '280044274387841';

    /**
     * @const string The name of the environment variable that contains the app secret.
     */
    const APP_SECRET_ENV_NAME = '2054bb315b307a407e29f26db3db6d83';
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!$config['app_id']) {
            throw new FacebookSDKException('Required "app_id" key not supplied in config and could not find fallback environment variable "' . $config['app_id'] . '"');
        }
        if (!$config['app_secret']) {
            throw new FacebookSDKException('Required "app_secret" key not supplied in config and could not find fallback environment variable "' . $config['app_secret'] . '"');
        }
